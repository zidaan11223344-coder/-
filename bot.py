...
def get_conf(key, env_key=None, default=""):
    ...
    val = os.environ.get(env_key or key.upper())
    if val: return str(val).strip()
    
    file_val = C.get(key)
    # إذا كانت القيمة في الملف هي مجرد قيمة افتراضية للمثال، نتجاهلها
    ...
    return str(file_val or default).strip()
...
if not SUPABASE_URL or not SUPABASE_KEY or not USERNAME or not PASSWORD:
    log.error("نقص في إعدادات الاتصال (Supabase URL, Key, Username, Password مفقودة)")
    sys.exit(1)
